return_list_merged_dataframes <- function(x, y, by = NULL,
                                           ...
                                           incomparables = NULL)
{
  ...
  list_out["merged"] <- df.merge

  # Create a subset of the merged data frame that came from x

  dX_merged <- return_merged_subset(df.merge = df.merge,
                                    "in.x")
  ...
  dY_merged <- return_merged_subset(df.merge = df.merge,
                                    "in.y")
...
#'
#' @return Returns the subset of dIn for which the value of varname is TRUE. This should indicate the rows of dIn that were merged-in the desired input data frame.
#'
#' @details None.
#'
#' @examples
#' d <- data.frame(id = 1:2, from.x = c(TRUE, NA), from.y = NA)
